...
│   ├── core/                   # 核心文档处理（图片+元数据）
│   ├── storage/                # 对象存储适配器
│   ├── asset/                  # 资产管理（上传、转移、下载、删除）
│   ├── template/               # 模板渲染引擎
│   ├── rule-engine/            # 规则引擎、内容变换、跨平台适配
│   ├── ai/                     # AI 能力：Slug 生成、内容校验
│   ├── fpe-wasm/               # FF1 格式保留加密（WASM）
│   ├── autocorrect-wasm/       # 文本自动纠正（WASM）
│   ├── cli/                    # 命令行工具
│   ├── mcp-server/             # MCP 服务器
│   ├── vscode-extension/       # VS Code 扩展
│   ├── markdown-it-presigned-url/           # Markdown-it 预签名 URL 插件
│   └── markdown-it-presigned-url-adapter-nodejs/  # Node.js 签名适配器
├── docs/                       # 项目文档
│   └── adr/                    # 架构决策记录
├── examples/                   # 使用示例
└── scripts/                    # 构建和发布脚本
...
- 命名规范详见 [DEV-013 §1](./DEV-013-api-conventions.md)
...
- [发布前校验与质量门禁指南](./DEV-005-publish-validation.md) - 包发布校验标准
