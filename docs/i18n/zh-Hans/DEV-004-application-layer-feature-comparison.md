...
| 功能 | CLI | VS Code | MCP Server | 共享库包 |
|------|:---:|:-------:|:----------:|----------|
| 章节编号管理 | `section-numbers add/remove` | `Add/Remove section numbers` / Rule | 无 | `@cmtx/rule-engine` |
...
| `section-numbers add/remove <file>` | 添加/移除章节编号 | `@cmtx/rule-engine` | formatter |
...
| ~~P3~~ | ~~章节编号~~ | ~~添加 `section-numbers` 命令（add/remove）~~ [OK] |
