...
**不适用**: `cmtx-vscode`（VS Code 扩展使用独立发布机制，参见 [`DEV-006-vscode-publish-guide.md`](../packages/vscode-extension/DEV-006-vscode-publish-guide.md)）

---

## 2. package.json 校验（npm 发布字段）

发布前重点校验 npm 包元数据字段，完整的 package.json 配置模板参见包开发指南。
...
> 手写 API 文档（`docs/i18n/*/api/` 目录下的文件）是对 TypeDoc 生成文档的补充说明，**每个 `##` 章节标题下的函数必须语义上属于该标题范畴**。

### 5.1. 章节与内容对应

章节标题应准确概括其下所有函数的核心功能，不得混入语义无关的函数。

| 错误示例 | 说明 |
|---------|------|
| `## 图片尺寸调整` 下包含 `parseImageElements` | parseImageElements 是解析函数，不属于"尺寸调整" |
| `## 图片尺寸调整` 下包含 `toHtmlImage` | toHtmlImage 是格式转换，不属于"尺寸调整" |

### 5.2. 校验方法

对照源码中的导出分组（`packages/*/src/index.ts` 中的注释分区）验证文档章节划分：

```bash
# 查看 index.ts 中的导出分组注释
grep -E '(// =+|=+>|// 图片)' packages/<name>/src/index.ts
```

### 5.3. 校验清单

- [ ] 每个 `##` 章节标题准确概括其下所有函数的核心功能
- [ ] 无语义无关的函数混入错误章节
- [ ] 若有函数从一个章节迁移到另一章节，同步移除原文残留
...
- [ ] TypeDoc 注释覆盖完整，`pnpm run docs:check` 无报错（参考 [第 6 节](#6-typedoc-校验)）
...
- [ ] 手写 API 文档章节标题与内容对应正确（参考 [第 5 节](#5-手写-api-文档校验)）
- [ ] 构建产物完整（参考 [第 7 节](#7-构建产物校验)）
- [ ] 远程仓库已更新，工作区干净（无未提交更改）
- [ ] 已登录 npm（`npm whoami`）
- [ ] 不是 VS Code 扩展（使用 `packages/vscode-extension/DEV-006-vscode-publish-guide.md`）

---

## 9. 参见

- [NPM 包发布指南](./DEV-006-publish_guide.md) — 完整的发布流程
