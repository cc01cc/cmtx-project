...
基于 Rust `fpe` crate 的 WebAssembly 实现，遵循 NIST SP 800-38G 标准的 FF1 格式保留加密算法。
...
**懒加载 + Promise 缓存机制**：首次调用加密功能时自动加载 WASM，无需手动 `loadWASM()`。如需提前加载或自定义 WASM 源，可调用 `loadWASM()`（幂等，多次调用只加载一次）。
