...
对象存储适配器，统一存储服务接口，支持多种云存储。

## 1. 功能特性

- **统一接口**：`StorageAdapter` 接口支持多种存储服务
...
凭证配置及环境变量完整清单请参考 [CFG-001 配置参考](../../CFG-001-configuration-reference.md)。
...
实现 `StorageAdapter` 接口以支持其他云存储服务：

```typescript
import type { StorageAdapter, AdapterUploadResult } from "@cmtx/storage";
