...
Agent: Calls scan.analyze to identify unused images, then delete.image with force=false for each
```

## 跨存储转移

```
User: "Transfer images from Aliyun OSS to Tencent COS"
Agent: Calls transfer.execute to move assets between providers
```
