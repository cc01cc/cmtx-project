...
| 启动开发服务器            | TERMINAL        | 运行 pnpm run dev               |
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 注册命令
    const disposable = vscode.commands.registerCommand("cmtx.image.upload", async () => {
...
#### 7.5.2. 模式 2：使用 LogOutputChannel（VS Code 1.74+）
