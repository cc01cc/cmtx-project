...
#### 3.5.3. Workspace Extension 的资源访问
...
#### 3.5.4. Workspace Extension 访问本地资源的特殊 API
...
| ----------------------------------- | -------------- | ------------------------------------------- |
| 访问 `~/Documents/file.txt`         | [OK] 直接访问 | [ ] 不能直接访问                           |
| 访问工作区文件 `/workspace/file.md` | [ ] 不能直接访问 | [OK] 直接访问                            |
| 执行 `git status`                   | 在本地执行     | 在远程执行                                  |
| 访问 `localhost:3000`               | 访问本地服务   | 访问远程服务（或通过 `asExternalUri` 转发） |
| 访问剪贴板                          | [OK]           | [OK]（通过 VS Code API）                   |
| 打开文件选择对话框                  | [OK]           | [OK]（通过 VS Code API）                   |

#### 3.5.6. Workspace Extension 的功能能力
...
| -------------- | -------- | -------------------------------- |
| 注册命令       | [OK]     | 完全支持                         |
| 使用快捷键     | [OK]     | 通过 `keybindings` contribution  |
| 修改文档内容   | [OK]     | 通过 `TextEditor.edit()`         |
| 访问编辑器     | [OK]     | `vscode.window.activeTextEditor` |
| 访问工作区文件 | [OK]     | 完全支持                         |
| 提供 CodeLens  | [OK]     | 完全支持                         |
| 提供代码补全   | [OK]     | 完全支持                         |
| 提供 Tree View | [OK]     | 完全支持                         |
| 显示通知       | [OK]     | 完全支持                         |
...
| **访问工作区文件** | [ ] 不能直接访问   | [OK] 可以直接访问     |
| **访问本地资源**   | [OK] 可以访问      | 有限（需要特殊 API）  |
...
| **可扩展性**       | 有限             | 完全自定义         | 完全自定义          |
| **markdown-it**    | [FAIL] 不支持 | [OK] 支持         | [OK] 使用 markdown-it |
| **自定义渲染规则** | [FAIL] 不支持 | [OK] 支持         | [OK] 支持             |
...
编辑器中打开的文件，其内存中的内容可能与磁盘上的内容不一致（用户编辑但未保存）。CMTX 的图片操作（上传、删除、转移）需要读取和修改文档内容，必须考虑此场景。

典型故障：用户编辑文档后未保存即执行上传 -> 上传管道读取磁盘旧版本 -> 选区偏移量在旧版本中对应不同内容 -> 操作失败或结果错误。

### 11.2. 解决方案：FileAccessor 接口

通过 `FileAccessor` 接口抽象文档读写，VS Code 扩展使用 `VSCodeFileAccessor` 实现：

- `readText`: 优先读取编辑器内存（`editor.document.getText()`），回退到磁盘
- `writeText`: 优先通过 `WorkspaceEdit` 写入（支持 undo/redo），回退到 `fs.writeFile`

```
FileAccessor (接口)
+-- FsFileAccessor      -> 读写磁盘（CLI/MCP Server）
+-- VSCodeFileAccessor  -> 读写编辑器内存，感知未保存内容
```
...
- 操作是否可能在编辑器环境中执行？
- 是否通过 `FileAccessor` 接口读写（而非直接 `fs.readFile`）？
- 写回时是否使用编辑器 API（而非直接 `fs.writeFile`）？
- 偏移量计算是否基于正确的文档内容（编辑器内存 vs 磁盘）？
