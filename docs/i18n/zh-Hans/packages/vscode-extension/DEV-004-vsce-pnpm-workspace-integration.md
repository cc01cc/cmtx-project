...
#### 2.3.1. 问题 1：npm list 无法解析 pnpm 的 symlink 结构
...
```json
{
    ...
    "scripts": {
        ...
        "publish:stable": "vsce publish --no-dependencies"
    },
    "dependencies": {
...
**可以**。peerDependencies 会被 tsdown 正常打包。

**原因**：

1. tsdown 的 `alwaysBundle` 会将所有非 `neverBundle` 的依赖打包进 `extension.js`
...
| 方案                         | 优点                                                                                                  | 缺点              |
| ---------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------- |
| **使用 `--no-dependencies`** | [OK] 简单，无需额外配置<br>[OK] 与现有 pnpm 工作流兼容<br>[OK] 打包速度快<br>[OK] 社区最佳实践                | [WARN] 需要理解原理   |
| **添加 yarn 构建**           | [FAIL] 增加维护成本<br>[FAIL] 需要维护两套锁文件<br>[FAIL] pnpm 和 yarn 依赖解析可能不一致<br>[FAIL] CI/CD 复杂度翻倍 | [FAIL] 不解决根本问题 |
...
- [tsdown 打包配置](https://tsdown.dev/options/deps)
