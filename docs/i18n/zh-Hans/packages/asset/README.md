...
内部 API（不从 barrel 导出，按需从模块直接导入）：
- `renderReplacementText(match, cloudResult, replaceOptions?, imageFormat?)` — 渲染替换文本（支持模板变量）
- `applyReplacementOps(content, ops)` — 按偏移量应用替换操作（已下沉至 `@cmtx/core`）
...
图片删除服务，内置引用检查和多种删除策略（trash/move/hard-delete）。

### @cmtx/asset/config - 配置管理

支持 YAML 配置文件加载、验证和环境变量替换。

### Service 层 - 统一服务接口

暴露 Service 层 API，作为 Rule 引擎的统一服务接口：

- **Service\<TConfig\>** 接口：所有服务的基础契约

## 核心理念

- **文件系统操作中心**：处理所有文件级操作（筛选、替换、删除、上传、下载）
...
`FileService` 封装了文件系统级别的图片处理（从 @cmtx/core 迁移）：
...
- **职责分离** - pipeline 只做上传和替换，写回和删除由调用者处理
...
```typescript
import { ConfigLoader, validateConfig } from "@cmtx/asset/config";
...
URL 检测工具已迁移至 `@cmtx/storage`，详见 [storage-url API 文档](/cmtx/api/storage-url/)：
...
```typescript
import { scanDirectory } from "@cmtx/asset";
import { sortByOrder } from "@cmtx/core";

// 扫描当前目录下所有 .md 文件
const docs = await scanDirectory("./docs");

// 按 sidebar_order 排序
const sorted = sortByOrder(docs);

// 只保留有 category 的文档
const categorized = docs.filter((d) => d.frontmatter.category);
```
...
interface ScanDirectoryOptions {
    recursive?: boolean;      // 是否递归子目录（默认 true）
    exclude?: string[];       // glob 排除模式
    include?: string[];       // glob 包含模式（默认 ["**/*.md"]）
    minFrontmatter?: boolean; // 是否要求至少有 frontmatter（默认 false）
}

await scanDirectory(dirPath: string, options?: ScanDirectoryOptions): Promise<DocEntry[]>
```
...
```typescript
interface DocEntry {
    filePath: string;                          // 相对于 dirPath 的文件路径
    frontmatter: Record<string, FrontmatterValue>; // 解析后的 frontmatter 字段
    title: string;                             // 文档标题（frontmatter.title > H1 > 文件名）
}
```
...
@cmtx/asset/upload 内置两层去重机制，提升上传效率：
...
各子模块的使用示例请参见对应包的 README 和 TypeDoc API 文档。
