...
> 最后更新：2026-05-16。新增包时按以下规则判断归属，并更新本清单。

**规则**：
- 库包（提供编程接口，被其他包 import）→ **changeset** 模式：`@cmtx/*` 下所有发布到 npm 的库包
- 应用层包（终端应用，不被其他包 import）→ **manual** 模式：`cmtx-vscode`、`@cmtx/cli`、`@cmtx/mcp-server`
- 切换轨道需经维护者同意并在 CHANGELOG 中注明

| 模式 | 包 | 说明 |
|------|----|------|
| changeset | `@cmtx/core` `@cmtx/asset` `@cmtx/storage` `@cmtx/template` | 核心库 |
| changeset | `@cmtx/fpe-wasm` `@cmtx/autocorrect-wasm` | WASM 包装器 |
| changeset | `@cmtx/rule-engine` `@cmtx/ai` | 处理引擎 |
| changeset | `@cmtx/markdown-it-presigned-url` `@cmtx/markdown-it-presigned-url-adapter-nodejs` | 插件 |
| manual | `cmtx-vscode` | VS Code 扩展（独立发布，不受 changeset 管理） |
...
> 通用 Markdown 格式规则见 `DOC-001`，本节仅定义 CHANGELOG 专有的条目规范。
...
#### 4.5.1. 步骤 1：验证标题格式与日期

确认版本标题格式为 `## [<version>] - YYYY-MM-DD`，与上一个版本的基准对齐。

#### 4.5.2. 步骤 2：对比 git diff 审核内容
...
#### 4.6.1. Before：`changeset version` 自动生成
...
#### 4.6.2. After：精修后（双语 + 标准分类，两条 changeset 合并）
...
- [包发布指南](./DEV-006-publish_guide.md) — 完整发布流程
