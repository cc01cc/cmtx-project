...
- 当前版本：`0.1.0-dev.20260412T12364`（开发版）
- 发布预览版时，手动修改为如 `0.1.0`
- 发布正式版时，手动修改为如 `0.2.0`
...
```bash
# 预览版
pnpm -F cmtx-vscode package:prerelease    # 打包预览版
pnpm -F cmtx-vscode publish:prerelease    # 发布预览版
...
- [ ] 更新 `CHANGELOG.md` 添加版本变更说明
- [ ] 运行测试并确保通过：`pnpm -F cmtx-vscode test`
- [ ] 确认 vsce 版本为最新：`npm install -g @vscode/vsce@latest`
- [ ] 确认已登录：`vsce login cc01cc`
...
# 3. 打包并发布预览版
pnpm -F cmtx-vscode package:prerelease
pnpm -F cmtx-vscode publish:prerelease
```
...
1. vsce 版本是否最新：`npm install -g @vscode/vsce@latest`
