...
suite("Upload Integration Tests", () => {
    ...
    suite("Naming Template Flow", () => {
        test("Complete config flow: VS Code setting -> ConfigBuilder", () => {
            const namingTemplate = "{name}-{md5_8}.{ext}";
            const prefix = "uploads/";

            const storageOptions = { prefix, namingTemplate };

            assert.strictEqual(storageOptions.namingTemplate, namingTemplate);
        ...
        test("Config flow with undefined namingTemplate", () => {
            ...
            const storageOptions = { prefix, namingTemplate: undefined };
    ...
    suite("ConfigBuilder Contract", () => {
        test("ConfigBuilder.storages should accept storagePool parameter", () => {
            // 验证 ConfigBuilder 的 storages 方法接受 storage pool
        ...
        test("ConfigBuilder should handle storages with prefix", () => {
            const builder = new ConfigBuilder();
