...
suite('Contract Tests - VS Code Extension <-> @cmtx/asset', () => {
    suite('ConfigBuilder.storage() Contract', () => {
        test('storage() should accept prefix and namingTemplate options', () => {
            // 这是 VS Code 扩展使用的接口
            ...
            }, 'ConfigBuilder.storage() should accept prefix and namingTemplate options');
        });

        test('storage() should handle undefined namingTemplate', () => {
            const builder = new ConfigBuilder();
            ...
            }, 'ConfigBuilder.storage() should handle undefined namingTemplate');
        });

        test('storage() should use namingTemplate consistently', () => {
            // 这个测试文档化命名映射关系
            ...
            // 不应该抛出错误
            assert.doesNotThrow(() => {
                builder.storage({} as any, options);
            }, 'ConfigBuilder.storage() should accept namingTemplate');

            // 验证使用了正确的参数名
            assert.strictEqual(options.namingTemplate, '{name}-{md5_8}.{ext}');
        });
    });
