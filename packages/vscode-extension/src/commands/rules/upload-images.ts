...
export async function uploadImagesRuleCommand(): Promise<void> {
    ...
    const ruleConfig = config?.rules?.["upload-images"];

    await executeRuleCommand("upload-images", ruleConfig);
}
