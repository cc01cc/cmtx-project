...
export async function frontmatterIdRuleCommand(): Promise<void> {
    ...
    const ruleConfig = config?.rules?.["frontmatter-id"];

    await executeRuleCommand("frontmatter-id", ruleConfig);
}
