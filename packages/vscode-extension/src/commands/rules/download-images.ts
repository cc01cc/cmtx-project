...
export async function downloadImagesRuleCommand(): Promise<void> {
    ...
    const ruleConfig = config?.rules?.["download-images"];

    await executeRuleCommand("download-images", ruleConfig);
}
