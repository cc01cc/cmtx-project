...
describe('AliOSSAdapter', () => {
    ...
    describe('getSignedUrl', () => {
        it('should generate signed URL successfully', async () => {
            ...
            vi.mocked(mockClient.signatureUrl).mockResolvedValue(mockUrl);

            const result = await adapter.getSignedUrl('test.png', 3600);

            expect(result).toBe(mockUrl);
            expect(mockClient.signatureUrl).toHaveBeenCalledWith('test.png', { expires: 3600 });
        });

        it('should throw error when generating signed URL fails', async () => {
            vi.mocked(mockClient.signatureUrl).mockRejectedValue(new Error('Invalid credentials'));
    ...
    describe('buildUrl', () => {
        ...
        it('should use default values when client options are missing', () => {
            const mockClientWithoutOptions = {
                ...mockClient,
            } as unknown as AliOSSClient;
        ...
        it('should handle client with empty options', () => {
            const mockClientWithEmptyOptions = {
                ...mockClient,
                options: {},
            } as unknown as AliOSSClient;
