...
import type { LocalImageMatchWithAbsPath, LoggerCallback, ReplaceOptions as CoreReplaceOptions } from '@cmtx/core';
import { replaceImagesInFile } from '@cmtx/core';
import type { ImageCloudMapBody, ReplaceOptions } from './types.js';
import { renderTemplateSimple, createContext } from './template-renderer.js';
...
function buildReplaceOperations(
    uploadedImages: Array<{ imageMatch: LocalImageMatchWithAbsPath; cloudResult: ImageCloudMapBody }>,
    replaceOptions: ReplaceOptions
): CoreReplaceOptions[] {
    ...
    for (const { imageMatch, cloudResult } of uploadedImages) {
        ...
        // 渲染每个字段的新值
        const newSrc = replaceOptions.fields.src ? renderTemplateSimple(replaceOptions.fields.src, renderContext) : undefined;
        const newAlt = replaceOptions.fields.alt ? renderTemplateSimple(replaceOptions.fields.alt, renderContext) : undefined;
        const newTitle = replaceOptions.fields.title ? renderTemplateSimple(replaceOptions.fields.title, renderContext) : undefined;
