...
describe('综合集成测试', () => {
    ...
    describe('配置构建验证', () => {
        it('应该正确构建和验证配置', () => {
            ...
            const config = new ConfigBuilder()
                .storage(
                    {
                        upload: async () => ({ name: 'test', url: 'https://mock.cdn.com/uploaded' }),
                    } as any,
                    {
                        prefix: 'blog/assets/',
        ...
        it('应该正确处理模板渲染', () => {
            ...
            logoImage.references.forEach((ref) => {
                ...
                const newSrc = renderTemplateSimple(config.replace!.fields.src, ctx);
                const newAlt = renderTemplateSimple(config.replace!.fields.alt, ctx);
    ...
    describe('端到端上传流程', () => {
        ...
        it('应该支持自定义命名模板', async () => {
            ...
            const config = new ConfigBuilder()
                .storage(mockAdapter, {
                    namingPattern: '{md5_8}_{fileName}'
                })
                .build();
            ...
            const logoUpload = mockAdapter.uploadedFiles.find(
                f => f.localPath.includes('logo.png')
            );
