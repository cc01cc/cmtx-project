...
import type { AdaptRule, RenderResult } from '../../types.js';
...
export function renderWechatMarkdown(markdown: string, rules: AdaptRule[]): RenderResult {
    const adapted = applyAdaptRules(markdown, rules);
    const content = renderMarkdownAsHtml(adapted.content, wechatTheme);
