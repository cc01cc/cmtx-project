...
import {
    createFF1Cipher as createBaseFF1Cipher,
    decryptString,
    encryptString,
    isWasmLoaded,
    loadWASM,
...
import { LuhnAlgorithm, generateCounterValue } from "@cmtx/core";
...
interface CounterGenerateOptions {
    length: number;
    radix: number;
}
...
export class IdGenerator {
    ...
    public encryptFF1(
        ...
        if (!isWasmLoaded()) {
            throw new Error("FF1 WASM not loaded. Call loadWASM() from @cmtx/fpe-wasm first.");
        }
        const cipher = createBaseFF1Cipher(prepareFPEKey(encryptionKey), radix);
    ...
    public decryptFF1(
        ...
        const cipher = createBaseFF1Cipher(prepareFPEKey(encryptionKey), radix);
        return decryptString(cipher, toDecrypt);
    }
    ...
    public validateEncryptedId(
        ...
        try {
            ...
            const cipher = createBaseFF1Cipher(prepareFPEKey(encryptionKey), radix);
            decryptString(cipher, normalizedId);
            return { valid: true, checksumValid: withChecksum };
        } catch {
            return { valid: false, checksumValid: false };
        }
    }
    ...
    public renderTemplateWithContext(template: string, context: TemplateRenderContext): string {
        return template.replace(/\{(\w+(?:_\w+)?)\}/g, (match, varName) => {
            ...
            if (counterMatch) {
                ...
                if (!context.counterService) return match;
            ...
            if (varName === "ff1") {
                if (!context.ff1 || !context.counterService) return match;
                const counterId = context.ff1.useCounter ?? "default";
                if (!context.counterConfigs?.[counterId]) return match;
    ...
    private createFF1Cipher(key: string | Buffer, radix: number = 36) {
        if (!isWasmLoaded()) {
            throw new Error("FF1 WASM not loaded. Call loadWASM() from @cmtx/fpe-wasm first.");
        }
        return createBaseFF1Cipher(prepareFPEKey(key), radix);
    }

    private encryptWithCipher(
        cipher: ReturnType<typeof createBaseFF1Cipher>,
        plaintext: string,
    ): string {
        return encryptString(cipher, plaintext);
    }

    private decryptWithCipher(
        cipher: ReturnType<typeof createBaseFF1Cipher>,
        ciphertext: string,
    ): string {
    ...
    public generateCounterValue(counterValue: number, options?: CounterGenerateOptions): string {
        return generateCounterValue(counterValue, options);
    }
    ...
    public static parseTemplate(
        template: string,
    ): Array<{ name: string; args: string[]; raw: string }> {
        const vars: Array<{ name: string; args: string[]; raw: string }> = [];
        const regex = /\{(\w+)(?:_(\d+))?\}/g;
        ...
        while ((match = regex.exec(template)) !== null) {
            ...
            vars.push({ name, args, raw: match[0] });
        }
        return vars;
    }
}
