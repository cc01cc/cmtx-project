...
import { uploadLocalImageInMarkdown } from "../src/upload/uploader.js";
...
describe("综合集成测试", () => {
    ...
    describe("配置构建验证", () => {
        it("应该正确构建和验证配置", () => {
            ...
            expect(config.prefix).toBe("blog/assets/");
        ...
        it("应该正确处理模板渲染", () => {
            ...
            // 验证模板渲染 - 使用 @cmtx/template/renderTemplate
            logoImage.references.forEach((ref) => {
                const ctx = {
                    cloudSrc: logoImage.cloudUrl!,
                    originalValue: ref.imageMatch.alt,
                };

                const newSrc = renderTemplate(config.replace?.fields.src, ctx, {
                    emptyString: "preserve",
                });
                const newAlt = renderTemplate(config.replace?.fields.alt, ctx, {
                    emptyString: "preserve",
                });
