...
describe('核心功能测试', () => {
    it('应该正确渲染模式', () => {
        ...
        const result = renderTemplateImage('{cloudSrc}', context);
        expect(result).toBe('https://cdn.example.com/test.png');

        const result2 = renderTemplateImage('{originalValue} - 已更新', context);
    ...
    it('应该正确处理字段分离', () => {
        ...
        mockImageReferences.forEach((ref) => {
            ...
            const newSrc = renderTemplateImage('{cloudSrc}', ctx);
            const newAlt = renderTemplateImage('{originalValue} [已处理]', ctx);
