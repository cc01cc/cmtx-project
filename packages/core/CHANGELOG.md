...
- **resize 模块**: 新增图片尺寸调整模块 (`resize.ts`)，包含以下导出：
  - `ImageElement` / `WidthDirection` — 类型定义
  - `parseImageElements` — 解析文本中的图片元素
  - `detectImageWidth` — 检测 HTML 图片宽度
  - `detectCurrentWidth` — 检测当前图片宽度
  - `calculateTargetWidth` — 计算目标宽度（支持 `"in"` / `"out"` 方向）
  - `resizeImageWidth` — 调整 HTML 图片宽度
  - `convertMarkdownImageToHtml` — Markdown 图片转 HTML
  - `convertMarkdownImageToHtmlWithWidth` — 转换并设置目标宽度
- `extractFrontmatterField`: 新增从 Markdown 文本中提取指定 Frontmatter 字段值的函数

### Changed

- **Logger**: 重构 Logger 模块，统一日志接口
  - `filterImagesInText` / `replaceImagesInText` 参数类型从 `LoggerCallback` 变更为 `Logger` 接口（**BREAKING**）
  - 使用 `dummyLogger` / `consoleLogger` 替代旧日志实现

### Removed

- **delete.ts 模块**: 移除整个删除模块（`deleteLocalImage`、`deleteLocalImageSafely`、`validatePathWithinRoot`）— 删除逻辑已移至 `@cmtx/asset` 层
- **目录级筛选函数**: 移除 `filterImagesFromFile`、`filterImagesFromDirectory` — 目录级操作已移至上层包
- **目录级替换函数**: 移除 `replaceImagesInFile`、`replaceImagesInDirectory` — 目录级操作已移至上层包
- **旧日志类型**: 移除 `LoggerCallback` 类型、`initLogger`、`getLogger`、`CmtxLogger` — 改用 `Logger` 接口和 `dummyLogger` / `consoleLogger`
- 移除废弃的示例脚本
...
- **resize module**: New image resizing module (`resize.ts`) with exports:
  - `ImageElement` / `WidthDirection` — type definitions
  - `parseImageElements` — parse image elements from text
  - `detectImageWidth` — detect HTML image width
  - `detectCurrentWidth` — detect current image width from elements
  - `calculateTargetWidth` — calculate target width (supports `"in"` / `"out"` direction)
  - `resizeImageWidth` — resize HTML image width
  - `convertMarkdownImageToHtml` — convert Markdown image to HTML tag
  - `convertMarkdownImageToHtmlWithWidth` — convert and set target width
- `extractFrontmatterField`: New function to extract a specific Frontmatter field value from Markdown text

### Changed

- **Logger**: Logger module refactored to unified interface
  - `filterImagesInText` / `replaceImagesInText` parameter type changed from `LoggerCallback` to `Logger` interface (**BREAKING**)
  - Replaced legacy logging with `dummyLogger` / `consoleLogger`

### Removed

- **delete.ts module**: Removed entire deletion module (`deleteLocalImage`, `deleteLocalImageSafely`, `validatePathWithinRoot`) — deletion logic moved to `@cmtx/asset` layer
- **Directory-level filtering**: Removed `filterImagesFromFile`, `filterImagesFromDirectory` — directory-level operations moved to upper-layer packages
- **Directory-level replacement**: Removed `replaceImagesInFile`, `replaceImagesInDirectory` — directory-level operations moved to upper-layer packages
...
- **Image filtering API refactored**: Filter parameters changed from `{ webHosts?, localPrefixes? }` to `{ mode, value }` (supports sourceType/hostname/absolutePath/regex modes)
- **Type definition refactored**: `ExtractOptions` and `ScanDirectoryOptions` restructured with changed parameter signatures
- **Location info removed**: Exact position information (line numbers, column numbers, etc.) no longer provided
- **Return types simplified**: Composite types like `DirectoryScanResult` simplified
...
- Fixed edge cases in path handling
- Optimized error handling for file read and write operations
- Improved cross-platform path compatibility
- Fixed memory issues with large file processing

### Performance Improvements

- Parsing performance improved via regex and caching
- Memory usage optimized by reducing unnecessary object creation
- Concurrency processing improved for directory scanning and batch operations
- Error recovery optimized through retry and degradation strategies

### Known Limitations

- File encoding: UTF-8 only, other encodings not supported
- Location info: exact line/column positions not provided
- Syntax support: standard Markdown and HTML image syntax only
...
- Fixed `relativePath` calculation in `replaceImageInFiles` (previously returned filename, now returns relative path from search directory)
- Fixed `replaceImageInFileInternal` function signature, added `searchDirAbsPath` parameter for correct relative path calculation

### Added

- Optional `logger` callback support for debugging and error tracking (`LoggerCallback` type)
- Added `logger?: LoggerCallback` optional parameter to all Options interfaces
- Input validation at API layer: empty string detection, negative depth detection, special character detection
- Added `validatePath` and `validateDepth` internal validation functions

### Changed

- API layer functions now throw explicit errors on invalid parameters (TypeError, RangeError)
- Improved error message readability and debugging experience
...
- **LSP standard positions**: 1-based lines, 0-based columns
- **Cross-platform**: Unified path handling, supports Windows and Linux
