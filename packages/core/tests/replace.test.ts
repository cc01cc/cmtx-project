...
describe('replaceImagesInFile - 文件验证', () => {
    it('应该拒绝目录路径', async () => {
        const result = await replaceImagesInFile('/tmp', [
            { field: 'src', pattern: './old.png', newSrc: './new.png' },
        ]);
    ...
    it('应该处理二进制文件警告', async () => {
        ...
        const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cmtx-test-'));
        const binaryFile = path.join(tempDir, 'test.png');
        await fs.writeFile(binaryFile, Buffer.from([0x89, 0x50, 0x4e, 0x47])); // PNG magic bytes
        ...
        const result = await replaceImagesInFile(
            binaryFile,
            [{ field: 'src', pattern: './old.png', newSrc: './new.png' }],
            logger
        );
        ...
        // 应该收到二进制文件警告
        expect(logs.some((log) => log.includes('二进制文件'))).toBe(true);
    });
    ...
    it('应该处理大文件警告', async () => {
        ...
        // 应该收到大文件警告
        expect(logs.some((log) => log.includes('大小') || log.includes('MB'))).toBe(true);
    });

    it('应该处理不支持的文件扩展名', async () => {
        const fs = await import('node:fs/promises');
        const path = await import('node:path');
        const os = await import('node:os');
        ...
        // 应该收到扩展名警告
        expect(logs.some((log) => log.includes('扩展名'))).toBe(true);
    });
});

describe('replaceImagesInDirectory - 错误处理', () => {
    it('应该处理 glob 错误', async () => {
        // 使用无效的模式应该返回空结果
        const result = await replaceImagesInDirectory(
            '/tmp',
            [{ field: 'src', pattern: './test.png', newSrc: './new.png' }],
            {
                patterns: ['invalid[pattern'], // 无效的正则表达式模式
            }
        );
    ...
    it('应该统计替换结果', async () => {
        ...
        await fs.writeFile(testFile, '![Image](./old.png)');

        const result = await replaceImagesInDirectory(tempDir, [
            { field: 'src', pattern: './old.png', newSrc: './new.png' },
        ]);
...
describe('replaceImagesInText - 边界情况', () => {
    ...
    it('应该处理包含 URL 编码字符的 src', () => {
        ...
        const result = replaceImagesInText(markdown, [
            { field: 'src', pattern: './image%20with%20spaces.png', newSrc: './new.png' },
        ]);
