...
describe("路径处理综合测试", () => {
  describe("绝对路径检测", () => {
    const testCases = [
      ...
      { path: "/home/user/documents", expected: true, description: "Unix用户目录路径" },
      { path: "~/Downloads/file.txt", expected: false, description: "波浪号路径(相对)" },
      
      // Windows路径
      { path: String.raw`C:\Windows\System32`, expected: true, description: "Windows盘符路径" },
      { path: String.raw`D:\Data\Files`, expected: true, description: "Windows数据盘路径" },
      { path: String.raw`\\Server\Share`, expected: true, description: "网络共享路径" },
      { path: String.raw`\Program Files\App`, expected: true, description: "Windows根目录路径" },
      
      // 混合格式
      { path: "C:/Program Files/Application", expected: true, description: "正斜杠Windows路径" },
      { path: "D:/data/projects", expected: true, description: "小写盘符路径" },
      
      // 相对路径
      { path: "./images/logo.png", expected: false, description: "当前目录相对路径" },
      { path: "../assets/banner.jpg", expected: false, description: "父目录相对路径" },
      { path: "images/photo.png", expected: false, description: "同级目录路径" },
