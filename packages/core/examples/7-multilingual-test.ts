/**
 * 示例 7: 多语言路径测试
 * 运行: pnpm exec tsx examples/7-multilingual-test.ts
 *
 * 测试系统对多语言路径的处理能力
...
async function detectLanguageByContent(content: string): Promise<string[]> {
...
async function main() {
  ...
  const languageSupport = {
    ...
    japanese: results.some(r => r.unicodePaths.some(p => /[\u3040-\u309f\u30a0-\u30ff]/.test(p))),
